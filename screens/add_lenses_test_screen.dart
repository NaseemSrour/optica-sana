...
class AddLensesTestScreen extends StatefulWidget {
  ...
  const AddLensesTestScreen({Key? key, required this.customer, required this.customerService}) : super(key: key);
...
class _AddLensesTestScreenState extends State<AddLensesTestScreen> {
  ...
  void initState() {
    super.initState();
    final sampleTest = ContactLensesTest(customerId: widget.customer.id!, examDate: DateTime.now());
  ...
  Future<void> _saveTest() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        final newTest = ContactLensesTest.fromMap(newMap.map((key, value) {
           if (value is String) {
            if (key.contains('date')) return MapEntry(key, value);
            final asDouble = double.tryParse(value);
            if (asDouble != null) {
              if (asDouble == asDouble.toInt()) return MapEntry(key, asDouble.toInt());
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('New contact lenses test saved successfully!')),
        );
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
