...
class _LensesHistoryScreenState extends State<LensesHistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    try {
      final tests = await widget.customerService.getContactLensesHistory(widget.customer.id!);
      setState(() {
        _tests = tests;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return FocusableActionDetector(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('Contact Lenses History${_isEditing ? " (Editing)" : ""}'),
          actions: [
            if (_tests.isNotEmpty)
              IconButton(
        ...
        body: _isLoading
            ...
            : _tests.isEmpty
                ? const Center(child: Text('No contact lenses history found for this customer.'))
                : Column(
                    children: [
  ...
  Widget _buildTestDataForm(ContactLensesTest test) {
    final fields = test.toMap().keys.where((k) => k != 'id' && k != 'customer_id' && k != 'exam_date').toList();
