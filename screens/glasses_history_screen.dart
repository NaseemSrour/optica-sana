...
class _GlassesHistoryScreenState extends State<GlassesHistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    try {
      final tests = await widget.customerService.getGlassesHistory(widget.customer.id!);
      setState(() {
        _tests = tests;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    return FocusableActionDetector(
      ...
      shortcuts: {
        ...
        LogicalKeySet(LogicalKeyboardKey.add): const NextIntent(),
        LogicalKeySet(LogicalKeyboardKey.numpadAdd): const NextIntent(),
        LogicalKeySet(LogicalKeyboardKey.subtract): const PreviousIntent(),
        LogicalKeySet(LogicalKeyboardKey.numpadSubtract): const PreviousIntent(),
      },
      actions: {
        EditIntent: CallbackAction<EditIntent>(onInvoke: (_) => _toggleEditMode()),
        SaveIntent: CallbackAction<SaveIntent>(onInvoke: (_) => _saveTest()),
        NextIntent: CallbackAction<NextIntent>(onInvoke: (_) => _navigateTests(-1)),
        PreviousIntent: CallbackAction<PreviousIntent>(onInvoke: (_) => _navigateTests(1)),
      },
      child: Scaffold(
        appBar: AppBar(
        ...
        body: _isLoading
            ...
            : _tests.isEmpty
                ? const Center(child: Text('No glasses history found for this customer.'))
                : Column(
                    children: [
  ...
  Widget _buildCustomerHeader() {
    return Padding(
      ...
      child: Wrap(
        ...
        runSpacing: 8,
        children: [
          _buildHeaderInfo('Name', '${widget.customer.fname} ${widget.customer.lname}'),
  ...
  Widget _buildTestDataForm(GlassesTest test) {
    final rightEyeFields = ['r_sphere', 'r_cylinder', 'r_axis', 'r_prism', 'r_base', 'r_va'];
    final leftEyeFields = ['l_sphere', 'l_cylinder', 'l_axis', 'l_prism', 'l_base', 'l_va'];
      ...
      child: Table(
        ...
        children: [
          TableRow(
            ...
            children: [
              const Padding(padding: EdgeInsets.all(8.0), child: Text('Eye', style: TextStyle(fontWeight: FontWeight.bold))),
              ...rightEyeFields.map((field) => Padding(
                padding: const EdgeInsets.all(8.0),
                child: Text(field.substring(2).replaceAll('_', ' ').toUpperCase(), textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold)),
              )),
            ],
          ),
  ...
  TableRow _buildEyeDataRow(String eyeLabel, List<String> fields) {
    return TableRow(
      children: [
        Padding(
          padding: const EdgeInsets.all(8.0),
          child: Text(eyeLabel, style: const TextStyle(fontWeight: FontWeight.bold)),
        ),
        ...fields.map((field) {
          return Padding(
